...
logistics = regression.DecisionTreeClassificationModelCreator(XTrain, yTrain, valueToPredict = XTest)
yPred = logistics[0]
classifier = logistics[1]

confusionMatrix = dataConverter.errorRateMatrix(yTest, yPred)

visualisation.visualiseColoredMap(XTrain, yTrain, ['red', 'green'], 'Decision Tree Classifier (Training set)', 'Age', 'Estimated Salary', classifier = classifier)
visualisation.visualiseColoredMap(XTest, yTest, ['blue', 'gray'], 'Decision Tree Classifier (Test set)', 'Age', 'Estimated Salary', classifier = classifier)
