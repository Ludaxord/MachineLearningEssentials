...
logistics = regression.SVMModelCreator(XTrain, yTrain, valueToPredict = XTest, kernel = 'rbf')
yPred = logistics[0]
classifier = logistics[1]

confusionMatrix = dataConverter.errorRateMatrix(yTest, yPred)

visualisation.visualiseColoredMap(XTrain, yTrain, ['red', 'green'], 'Kernel SVM (Training set)', 'Age', 'Estimated Salary', classifier = classifier)
visualisation.visualiseColoredMap(XTest, yTest, ['blue', 'gray'], 'Kernel SVM (Test set)', 'Age', 'Estimated Salary', classifier = classifier)
