...
logistics = regression.LogisticRegressionModelCreator(XTrain, yTrain, valueToPredict = XTest)
yPred = logistics[0]
classifier = logistics[1]

confusionMatrix = dataConverter.errorRateMatrix(yTest, yPred)

visualisation.visualiseColoredMap(XTrain, yTrain, ['red', 'green'], 'Logistic Regression (Training set)', 'Age', 'Estimated Salary', classifier = classifier)
visualisation.visualiseColoredMap(XTest, yTest, ['blue', 'gray'], 'Logistic Regression (Test set)', 'Age', 'Estimated Salary', classifier = classifier)
